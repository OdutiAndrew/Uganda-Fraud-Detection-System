...
st.set_page_config(
    page_title="Uganda Fraud Detector",
    page_icon="S",
    layout="centered"
)
...
with col1:
    ...
    network = st.selectbox(
        "Network Provider",
        ['MTN MoMo', 'Airtel Money', 'SACCO Account']
    )

    district = st.selectbox(
        "District",
        [
            'Kampala', 'Wakiso', 'Mukono', 'Gulu',
            'Mbarara', 'Jinja', 'Mbale', 'Lira',
            'Arua', 'Fort Portal'
        ]
    )

    amount = st.number_input(
        "Transaction Amount (UGX)",
        min_value=1000,
        max_value=5_000_000,
        value=200_000,
        step=10_000
...
if st.button("Analyse Transaction", type="primary", use_container_width=True):
    ...
    try:
        ...
        amount_tier = min(
            4,
            int(np.digitize(amount, [50_000, 200_000, 500_000, 1_000_000]))
        )
        ...
        row.update({
            ...
            'prev_txn_count_30d': prev_txn_30d,
            'failed_pin_attempts': failed_pin,
            'is_new_recipient': int(is_new_recip),
            'same_network': int(same_net),
            'device_change': int(device_chg),
            'is_weekend': int(is_weekend_chk),
            'is_night': is_night,
            'amount_tier': amount_tier,
            'new_account_large_txn': nalt,
            'risk_score': risk_score
        })

        # One-hot encoding
        for col_name, val_name in [
        ...
        if pred == 1:
            ...
            st.markdown(
                "This transaction is flagged as **HIGH RISK**. "
                "Recommend blocking for review."
            )
        else:
            st.success(f"LEGITIMATE - Risk Score: {proba*100:.1f}%")
            st.markdown(
                "Transaction appears **LOW RISK** and is approved."
            )
